...
export async function fetchPayrollData(): Promise<PayrollEntry[]> {
  ...
  const voluntaryDeduction = voluntary?.reduce((sum, d) => sum + Number(d.amount ?? 0), 0)
  const loanInstallments = loans?.reduce((sum, l) => sum + Number(l.principalrepaid ?? 0), 0)
  const advanceAmounts = advances?.reduce((sum, a) => sum + Number(a.amount ?? 0), 0)

  const total_deductions = voluntaryDeduction + loanInstallments + advanceAmounts
