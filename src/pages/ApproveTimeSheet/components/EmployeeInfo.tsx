...
import { IdCard, User, Briefcase, Building, ClockIcon, MapPin } from "lucide-react";
...
<<<<<<< HEAD
  ...
  const getAttendance = async () => {
    const records = await getAttendanceHistory(employee_schedule_id, employeeID);
    setAttendance(records || []);
  }

  const handleApprovedAttendance = async (attendance_id: number, status: string) => {
  ...
  return (
    ...
    <div className="lg:w-1/3 bg-white p-4 space-y-4">
      <div className="flex items-center space-x-3">
        ...
        <span className="text-lg font-semibold">ID: {employee_id}</span>
      </div>
      <div className="flex items-center space-x-3">
        <User className="h-6 w-6 text-green-500" />
        <span className="text-lg font-semibold">{first_name} {last_name}</span>
      </div>
      <div className="flex items-center space-x-3">
      ...
      <div className="flex items-center space-x-3">
        ...
        <span className="text-base">{shift_start_time} - {shift_end_time}</span>
      </div>

      <div className="mt-3 p-3 bg-blue-50 rounded-lg shadow-inner">
        <h3 className="text-base font-semibold text-blue-800 mb-2">Compensation</h3>
        ...
        <table className="min-w-full bg-white rounded-2xl shadow">
          <thead>
            <tr className="bg-indigo-100">
              {["DATE", "CLOCK IN", "TIME OUT", "TOTAL HOURS", "OT", "STATUS"].map((label) => (
                <th
                  key={label}
                  className="py-3 px-6 text-left text-sm font-semibold uppercase text-indigo-700 tracking-wide"
                >
                  {label}
                </th>
              ))}
          ...
          <tbody>
            {attendance.map((rec, idx) => (
              ...
              >
                <td className="py-3 px-6 text-sm text-gray-700">{new Date(rec.attendancedate).toLocaleString("en-US", {
                  year: "numeric",
                  month: "long",
                  day: "numeric"
                })}</td>
                ...
                <td className="py-3 px-6">
                  <span
                    className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${statusStyles[rec.status] ?? "bg-gray-100 text-gray-800"}`}
                  >
                    {rec.status}
                  </span>
                </td>
        ...
        >
          ...
          >
            ...
            <div className="bg-gray-100 px-4 py-2 rounded-lg">
              <h4 className="text-sm font-medium text-gray-600 uppercase">Attendance ID # {selectedRecord.attendanceid}</h4>
            </div>
            ...
            <p className="ml-8 text-gray-700 mb-4">{selectedRecord.loc_clock_in}</p>

            <div className="flex items-center space-x-2 mb-3">
              <MapPin className="h-6 w-6 text-red-500 rotate-45" />
              <span className="font-medium">Clock Out:</span>
            </div>
            <p className="ml-8 text-gray-700 mb-6">{selectedRecord.loc_clock_out}</p>

            {/* Action Buttons */}
            <div className="flex justify-end space-x-4">
              <button
                onClick={() => {handleApprovedAttendance(parseInt(selectedRecord.attendanceid!, 10), 'Rejected')}}
                className="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition"
              >
                Reject
              </button>
              <button
                onClick={() => {handleApprovedAttendance(parseInt(selectedRecord.attendanceid!, 10), 'Approved')}}
